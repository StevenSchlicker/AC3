\begin{pa} \label{PA:10.3} Consider the kinetic energy function $f$ from Preview Activity \ref{PA:10.2} defined by
\[f(m,v) = \frac{1}{2}mv^2,\]
...
Since $f_v(m,v) = mv$ it follows that $f_{vv}(m,v) = m$. The units of $f_v$ are $\frac{\text{J}}{\text{m/s}}$, so the units of $f_{vv}$ are $\frac{\frac{\text{J}}{\text{m/s}}}{m/s}$.

\end{comment}

    \item What, if anything, do you notice about $f_{mv}(m,v)$ and $f_{vm}(m,v)$?

\begin{comment}

The only thing to notice is that $f_{mv}(m,v) = v$ and $f_{vm}(m,v)= v$, and so $f_{mv}(m,v) = f_{vm}(m,v)$, although their units are different.
